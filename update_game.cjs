...
const htmlEnd = lines.findIndex(l => l.includes('<div id="fake-loader"'));
const jsStart = lines.findIndex(l => l.includes('// --- MINI GAME LOGIC ---'));
...
const newGameHTML = `    <!-- Mini Game Hint -->
    ...
    <div id="game-overlay" class="fixed inset-0 z-[99999] bg-darkBg hidden flex-col items-center justify-center pointer-events-auto overflow-hidden">
        ...
        <div class="relative w-full max-w-3xl h-[70vh] md:h-[80vh] border-x border-white/20 overflow-hidden bg-black flex flex-col items-center" id="game-area" style="touch-action: none;">
            
            <!-- Speed lines illusion for falling -->
            <div id="speed-lines" class="absolute inset-0 opacity-20 pointer-events-none" style="background: repeating-linear-gradient(180deg, transparent, transparent 40px, rgba(255,255,255,0.05) 40px, rgba(255,255,255,0.05) 80px); animation: speedDrift 0.3s linear infinite;"></div>
            
            <div class="absolute top-6 right-6 font-mono text-xl md:text-2xl text-lightText font-bold z-10 drop-shadow-md" id="game-score">00000</div>
            ...
            <div id="game-start-msg" class="absolute inset-0 flex flex-col items-center justify-center bg-darkBg/80 z-20">
                ...
                <button id="start-btn-click" class="mt-8 font-mono text-xs md:text-sm bg-simbionBlue text-white px-6 py-2 rounded uppercase tracking-widest hover:bg-blue-600 transition-colors">START FALLING</button>
            </div>
            
            <div id="game-over-msg" class="absolute inset-0 flex flex-col items-center justify-center bg-darkBg/90 z-20 hidden">
                <p class="font-mono text-2xl md:text-4xl text-simbionBlue font-bold mb-2 uppercase">CRASHED!</p>
                <p class="font-mono text-sm md:text-base text-lightText mb-6 text-center px-4">YOU HIT AN OBSTACLE.</p>
            ...
            <div id="game-player" class="absolute top-[10%] md:top-[15%] left-1/2 -translate-x-1/2 w-[70px] md:w-[90px] h-auto z-10 will-change-transform pointer-events-none">
...
const newGameJS = `            // --- MINI GAME LOGIC ---
            ...
            function resetGame() {
                score = 0;
                fallSpeed = window.innerWidth < 768 ? 5 : 7;
                playerX = 0;
                targetPlayerX = 0;
                frameCount = 0;
            ...
            function createObstacle() {
                ...
                const colors = ['bg-red-600 border-red-400', 'bg-white border-gray-300', 'bg-simbionBlue border-blue-400'];
                const cls = colors[Math.floor(Math.random() * colors.length)];
                
                el.className = \`absolute border-2 \${cls}\`;
                el.style.width = obsWidth + 'px';
                el.style.height = obsWidth + 'px';
                
                // Spawn at the BOTTOM of the game area, moving UP
                const areaWidth = gameArea.clientWidth;
                // Add some padding to avoid spawning out of bounds
                const x = Math.random() * (areaWidth - obsWidth - 20) + 10;
            ...
            function updateGame() {
                ...
                frameCount++;
                const spawnRate = Math.max(15, Math.floor(45 - (frameCount / 150)));
                ...
                const shrink = window.innerWidth < 768 ? 10 : 15;
